...
|---|---|---|---|---|---|---|
| 1 | **Train on what is scored: weight the ahead+imm losses by `label_is_equalize` (unscored rows x 0.25)** -- the never-scored first sixth of each history is EASIER (by-user BCE 0.189 vs 0.279, fail 7% vs 13-15%) and takes ~17% of the fit | invented | both | +0.0000..+0.0003 | 1 run | **REPORTED 09-07 11:05 = iter 68, REJECTED -- a REGRESSION in BOTH modes** (-0.000173 / -0.000184 at p_worse 5e-07 / 3e-37), i.e. the pre-registered P3: the never-scored early rows are load-bearing. **In a recurrent model an unscored prefix row is the computation path to every scored row after it**, so reweighting rows is not the i.i.d. operation it looks like. **Closes the row-reweighting family (with iter 37) and kills the cheap recovery-window proxy for the chunk mismatch.** (was: BUILT + ARMED 09-06 05:05 (`scratchpad/eqw/`: `RWKV_EQUALIZE_LOSS_W`, smoke 9/9 on a mixed chunk, PREREG, `mk_eqw.py <realcyc\|hord\|muonscale>`, `auto_control.py` picks the base on muonscale's verdict, waiter on `muonscale.log`) |
| 2 | Learned initial state per stream (RWKV "state tuning") | adopted | both | ~0 by the metric route | 1 run + trace + Rust port | **DEMOTED 09-07 -- the metric route is DEAD.** The eval does NOT chunk (test db: median 1 chunk/user, 0.2% multi-chunk, **0.00% of rows near an interior boundary**), so a learned init recovers **+0.000000** there. What survives is a training-conditioning argument only, at real deploy debt. `scratchpad/proposals_2026-09-06/CHUNKING_CORRECTION.md` |
...
| 4 | **Chunk-continuous training (state carry)** | adopted | both | now evidence-backed, size unknown | multi-day, **but the CUDA kernel is DONE + parity-verified** | **PROMOTED 09-07 on a measured train/eval mismatch: the train db cuts at ~16,384 (88.3% of users multi-chunk, 12.50% of rows in a cold-start window at +0.004..+0.012 ahead each) while the eval scores 51k-median histories in ONE sequence.** `STATEFUL_BPTT_PLAN.md` was shelved on SPEED and its accuracy case was never quantified; steps 1-3 remain. **Andrew's call on the days.** A ~5-line proxy (down-weight the post-boundary recovery rows, the eqw instrument) tests the hypothesis with no kernel work -- sequence it after eqw |
...
* **Rank 5 (hord) REPORTED 09-06 14:38 = iter 66, REJECTED -- a tie leaning POSITIVE in both modes** (vs realcyc ahead +0.000069 p=1.7e-3 / imm +0.000060 p=1.6e-4; both inside the floor). **P3 held with margin** (crossing rates −71..−85% at every horizon, label-t 29.9% → 5.8%) so the pre-registered falsifier P4 fires: ordering the counterfactuals off the label horizon carries no information about the pressed curve at the label horizon. No dose retry. Curve-shape family 2/5; graft INGREDIENT if another sub-bar positive appears in the same mode. Detail: `research_5k_verbose.md` iter 66.
* **Rank 6 (SAM) REPORTED 09-06 04:17 = iter 65, REJECTED -- a REGRESSION in both modes** (vs realcyc ahead −0.000277 at p_worse 5e-86, imm −0.000155; past the 0.0002 abort line) at a demonstrably engaged dose (median sharpness gap@0.05 0.0230 → 0.0133). **Train loss rose with held-out loss** (last 300 decay steps ahead 0.31807 vs 0.31773): the flatter minimum is a worse fit, not a better-generalising one -- at 1.25 epochs the model is fit-limited and there is no gap for flatness to close. The halved-rho retry is registered but demoted. **Explicit regularisers at this budget are now deprioritised as a family** (dropout ↓, wd ↑, lorawd, SAM all null-or-worse); re-screen on the endgame's checkpoints, where a gap can exist. Same night, three CPU screens closed neighbours before GPU: **PCGrad DEAD** (no ahead/imm gradient conflict in the trunk, 0 of 6 chunks), **LAWA/WS-checkpoint averaging DEAD** (the average of the last 2-6 WS checkpoints is worse than the WS-final; consecutive displacements near-orthogonal), **cold-grade probe DEAD** (the trunk carries no grade beyond R). Detail: `research_5k_verbose.md` iter 65.
...
* **Rank 11 (Muon coverage of the 26 `*scale*` matrices) ALIVE 09-05 19:10** (`scale_probe.py`, realcyc rc_ws_50 -> rc_d_10935): update anisotropy sigma_max/||dW||_F median **0.653** (LoRAs 0.649 -- the population iter 53 paid on; white-noise reference 0.447); but they carry **1.0% of the update energy** (LoRAs 42%), so the proportional expectation is ~1/40 of iter 53's +0.00017 = sub-bar. Queued as the ADOPTED slot after hord only because it is the last screened adopted candidate; a 2-line lever (`RWKV_MUON_INCLUDE_SCALE=1`, own wd=0 group) with a param-identity smoke, to be built while hord runs.
* **Rank 10 (curve-logit recalibration) DEMOTED 09-05 19:05** (`recal_probe.py`, realcyc's 218,841 train-user records, by-user 2-fold): shift-only held-out prize **+0.000589 / -0.000145** across the two folds (fitted shifts -0.07 vs -0.16 -- the optimum is user-set-dependent), mean +0.000222; shift+temperature +0.000054 (under the kill line). Calibration gap flat across review index (-0.002..-0.010). Not robust enough for a 4 h eval-only slot; revisit only with a >=30-user fit.
**Rank 1 (ordcut) REPORTED 09-05 18:49 = iter 64, REJECTED -- a LARGE ahead regression (-0.003181 at p_worse 5e-163, 16x the abort line; imm -0.000116) at full engagement (cut a = 0.93 logits; AUC Good-vs-Hard on the candidate's own R 0.851 vs 0.737).** Mechanism: CORAL's shared logit cannot serve the pass/fail and the Hard/Good boundaries at once here (the Easy share is U-shaped in R); at lambda 0.25 (~4x the ahead BCE at init) the ordinal side won. **The PREREG's one retry (lambda 0.1, min_t 3 d) is REGISTERED at the BOTTOM of the queue: if run, decouple the scale (`z_ord = s*z - a`, +1 train-only param) -- a different constraint -- and only if nothing better is queued.** sam launched on realcyc by auto_control (118 s after the marker).
**Rank 2 (durdrop) REPORTED 09-05 08:38 = iter 63, REJECTED by the pre-registered regression rule** (imm -0.000370 at p_worse 8e-120; ahead -0.000130; engagement -37%, short of the +0.0007 line). The duration input serves the rating head too, and that head pays. Family CLOSED 0/3 (18, 33, 63); **rank 9 (probe density 0.20) DEMOTED with it** -- it is the PAVA-path restriction of the same withholding. ordcut launched on realcyc by auto_control (78 s after the marker).
...
| 5c | **feat_loo** (~8 h) | leave-one-out: 19 arms, one per feature (sin/cos pairs together), 300 users each | Andrew 2026-09-03 ("ideally ablate everything") | featB | **DONE 09-04 22:22** -- t_since_any = the whole clock gain; sibling_gap / creation_to_first_review / creation-batch columns real (+0.0002..+0.0005); ten columns at a uniform ~+0.00008 ahead = the zero-any-column floor. No drop run queued (no accuracy to gain); `scratchpad/feat_loo/` |
