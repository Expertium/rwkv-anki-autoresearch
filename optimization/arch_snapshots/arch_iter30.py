...
# iter29 (CHAMPION): SRS heads halved 128->64 (num_curves/num_points below) -> 192,800 params, state
# 12.75 KiB. iter30 (THIS): iter29 + card stream 3->2 layers -> per-card STATE 12.75->8.5 KiB (the
# state gate; only the card_id stream counts). The state cut Andrew greenlit; ~0.006 imm cost fits the
# iter29 budget (headroom 0.0080). REMINDER: champion = iter29 (arch_snapshots/arch_iter29.py, card=3);
# restore it if iter30 fails the gate.
