...
| **realcyc** | 1-5000 | 0.2981ᵍ⁵ | 0.2636ᵍ⁵ | +0.0035 / +0.0000 | 0/2499 | **rejected — exact TIE; layout ADOPTED by directive** | 0.92 / 0.997 (vs gen4base) | 563,652 | 0 | Andrew | **ᵍ⁵ = REAL-TIME CYCLES (`RWKV_REAL_CYCLES=1`, gen-5 dbs): 24 UTC-anchored real cycle columns replace the 28 pseudo day-offset dims + pseudo `day_of_week`; input 114→109.** vs gen4base: ahead +0.000007 / imm −0.000045, both inside the ±7.5e-5 floor — the pre-registered P3 outcome (PREREG written before any number). P2 refuted too (no concentration in long-history users, rho −0.015). => real calendar phase carries nothing the query row's clock columns do not already give; the pseudo cycles were dead weight, so the swap is a FREE simplification. ⚠ imm harm test rank-significant (p_worse 0.0028) at a magnitude inside the floor (the iter-44 shape). **ADOPTED as the lineage's input layout on Andrew's directive: gen 5 (on the SSD, 0.95 steps/s) is the features lineage's db and this row is the reference for every later gen-5 candidate; lorawd regenerated on it.** Reversible (gen 4 kept). |
| **62** `lorawd` | 1-5000 | 0.2981ᵍ⁵ | 0.2636ᵍ⁵ | +0.0035 / +0.0001 | 0/2499 | **rejected** (tie, leaning negative) | 0.21 / 0.996 (vs realcyc) | 563,652 | 0 | adopted (Moonlight 2502.16982) | **Decoupled weight decay on the LoRA Muon group only, `RWKV_MUON_LORA_WD=0.05`** -- the brake iter 53's +62% LoRA-norm growth seemed to call for. vs realcyc: ahead −0.000040 (p_worse 0.79) / imm −0.000024 (p_worse 0.004, rank-significant inside the floor). **P3 holds, measured before the number:** the lever moved the LoRA norm ratio to 0.868 at WS end and 0.811 deployed, monotonically -- an engaged dose, not an inert flag. => the growth is gradient-driven and restoring (a decay that would shrink a free weight 75% trimmed it 19%), and damping it buys nothing. **Endgame consequence: no brake on the LoRA group by default; re-measure on that run's checkpoints.** No 0.2 retry (sign already negative both modes). Optimizer family 2/4: coverage pays, descent quality and norm control do not. |
| **63** `durdrop` | 1-5000 | 0.2982ᵍ⁵ | 0.2640ᵍ⁵ | +0.0036 / +0.0004 | 0/2499 | **rejected** (imm REGRESSION) | 0.90 / 1.0 (vs realcyc) | 563,652 | 0 | invented | **Duration dropout `RWKV_DUR_DROP=0.25`** -- train-only zeroing of the current review's duration input, iter 33's prescribed clean retry, aimed at the 70% half of the ahead-only rectification penalty (screen: +0.001388 ahead reliance on realcyc). vs realcyc: ahead −0.000130 (p_worse 0.10) / **imm −0.000370 at p_worse 8e-120** -- the pre-registered imm harm line (−0.0001) and abort line (−0.0002) both crossed. **P3 measured before the number: engaged, −37% duration reliance, short of the +0.0007 line.** Mechanism = the pre-registered P2 risk: the STATE loses duration on a quarter of rows and the rating head pays (iter 18 measured duration as real imm signal). **Family CLOSED 0/3 (18, 33, 63) -- the deploy rectification penalty is a fact of deploy, not a training target.** No p=0.5 retry (regression, not null). |
| **64** `ordcut` | 1-5000 | 0.3013ᵍ⁵ | 0.2637ᵍ⁵ | +0.0067 / +0.0001 | 0/2499 | **rejected** (ahead REGRESSION −0.0032) | 1.0 / 1.0 (vs realcyc) | 563,654 | 0 | adopted (CORAL 1901.07884) | **Ordinal one-cut supervision of the curve logit from the NEXT rating (`RWKV_ORD_LAMBDA=0.25`)** -- the unused k+1 button as a graded target; two agents proposed it independently. vs realcyc: **ahead −0.003181 at p_worse 5e-163** (16× the abort line), imm −0.000116. **Engaged with margin (both halves recorded before the number):** cut a → 0.93 logits, AUC(Good vs Hard) on the candidate's own R 0.851 vs 0.737. => the term did what it was asked and that is the cost: CORAL's shared logit forces one z to serve the pass/fail AND the Hard/Good boundary, and at λ 0.25 (~4× the ahead BCE at init) the ordinal side won, re-shaping z for ranking at the expense of binary calibration. A mechanism result, not a dose result. The PREREG's one retry (λ 0.1, min_t 3 d) is registered but demoted; if ever run, decouple the scale (z_ord = s·z − cut). Family label-side curve supervision 0/1. Two guards fired and were fixed the same day: a hollow first launch (swallowed per-batch exception) and a deploy loader refusing train-only keys. |
| **65** `sam` | 1-5000 | 0.2984ᵍ⁵ | 0.2637ᵍ⁵ | +0.0037 / +0.0002 | 0/2499 | **rejected** (REGRESSION both modes) | 1.0 / 1.0 (vs realcyc) | 563,652 | 0 | adopted (SAM 2010.01412) | **Sharpness-Aware Minimization, decay-only, rho 0.05 (`RWKV_SAM_RHO`)** on realcyc's WS-final; control = realcyc's own decay. vs realcyc: **ahead −0.000277 at p_worse 5e-86**, imm −0.000155 at p_worse 7e-157 -- past the 0.0002 abort line. **Engaged as designed:** the sharpness probe's median gap@0.05 fell 0.0230 → 0.0133 (−42%). **And the train loss rose with it** (last 300 decay steps: ahead 0.31807 vs 0.31773; 10-user val 0.3215 vs 0.3212): train and held-out moved together, so the flatter minimum is a worse fit, not a better-generalising one -- at 1.25 epochs the model is fit-limited and a regulariser that trades fit for flatness has nothing to buy. The PREREG retry (halved rho) is registered but demoted. Same night, three CPU screens closed neighbours before GPU: PCGrad dead (no ahead/imm gradient conflict), LAWA checkpoint averaging dead (the WS-average is worse than the WS-final), cold-grade probe dead. |
| **66** `hord` | 1-5000 | 0.2980ᵍ⁵ | 0.2635ᵍ⁵ | +0.0034 / −0.0000 | 0/2499 | **rejected** (tie, leaning positive) | 0.0017 / 0.00016 (vs realcyc) | 563,652 | 0 | invented | **Multi-horizon button-order hinge on the 4 probe curves (`RWKV_PAVA_HORIZON_LAMBDA=0.05`, t×1/8 and t×8)**; control realcyc, curve-side gate. vs realcyc: ahead **+0.000069** (p=1.7e-3) / imm **+0.000060** (p=1.6e-4) -- both inside the floor, both under the bar. **Engaged with margin (P3 recorded before the number):** off-label crossing rates fell 32.5/32.1/35.6/48.8% → 9.5/6.6/5.4/7.6%, label-t 29.9% → 5.8%. => the pre-registered falsifier P4: ordering the counterfactuals off the label horizon carries no information about the pressed curve at the label horizon. No dose retry (reserved for a P3 failure). Curve-shape constraints 2/5. Graft ingredient if another sub-bar positive appears in the same mode. Milestone: imm 0.263533 is the first gen-5 number below the old d=128 model's VAL-half imm (0.263561). |
