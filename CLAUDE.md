...
#### LIVE
**▶ BUDGET CALIBRATION RUNNING** (launched 2026-08-10 19:46, `scratchpad/budgetcal/run_cal.cmd`): three arms at 1/3 training budget over recipes whose full-budget answers are known -- c41 (champion), c42 (order-only, a known large loss), c43 (interleave at the original order, a VERIFIED NULL). c41-vs-c43 measures the short-budget NOISE FLOOR; c41-vs-c42 tests whether a large effect keeps its sign. ~15.3 h. Purpose: Andrew 2026-08-10, *"it's easier to cut the epoch budget down by a factor of 3-4 than to improve the % of good ideas by a factor of 3-4"*. ⚠ A budget cut ALONE buys only 1.8x, not 3x -- eval is a fixed 2.9 h and becomes 57% of an iteration; the 2.6x design needs the ranking eval halved to 1250 users (variance-only, bias-free) with the full 2500 kept for promotion. ⚠ These three arms are all SCHEDULE changes, so they CANNOT detect the bias that matters most: short budgets favour early-learning changes and penalise regularization/capacity (same mechanism as the vprune scope lesson).
**▶ ITER 45 STAGED, NOT LAUNCHED** (`scratchpad/iter45_kddecay/run_iter45.cmd`): KD through the DECAY phase -- winner of the 15-proposal ranking, and ZERO code (the runner simply does not clear `RWKV_KD_MIX`, and sets alpha 0.9 -> 0.5 for decay). Since iter 34 adopted decay_ratio=1.0, decay is HALF of all training and runs on pure hard labels. Self-validating: the per-step `labels_sum` checksum hard-exits 43 if the decay batch stream does not match the dump. Set `MAXSTEPS` in the runner AFTER the calibration reports (0 = full budget, gate vs iter 41; 3645 = 1/3, gate vs the calibration's c41, which is the same champion recipe at that budget).
...
1. **The algorithmic loop** (the endgame's step 1) -- iter 45 staged, ranked list of 14 further proposals in this session's record. Idea generation now uses Andrew's 3-agent protocol (2026-08-10): three subagents with DIFFERENT priors (literature / domain / reject-log steelman) each write 5 proposals from >=2 families, then all 15 are ranked and the top one is implemented.
...
4. **Rust port** (`rust/rwkv-infer/TRACK2_PORT_PLAN.md`) -- gaps 7/8 (interleave schedule, stream order) added 2026-08-10; gap 8 may be droppable since iter 43 showed the reorder is free. Remaining measured items and the AGPL/SIMD note are in that plan.
