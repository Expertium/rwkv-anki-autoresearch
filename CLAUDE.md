...
#### LIVE
**⟶ 2026-09-11 21:14 -- ★★ ARM 2 IS IN: THE QAT TAX AT 10 EPOCHS IS +0.002529 ahead / +0.004466 imm** (deployed 0.300107 / 0.266532 vs arm 1's 0.297577 / 0.262066; n=2,499, size 0/2,499; 84.7% / 97.9% of users worse). Single variable vs arm 1, evaluated with the LEARNED step-21870 catalogs (probe-proven, no post-check NOTE). **Ahead's tax is budget-independent (1.25 ep: +0.002286); imm's GREW 28% (+0.003486).** All four PREREG questions held. **The deployed 10-epoch model is WORSE than the plain 1.25-epoch realcyc in both modes** -- the tax is ~5x the budget gain on ahead, ~3x on imm -- and misses the stop criterion by 0.0051 / 0.0025. **=> THE QAT TAX IS NOW THE PROJECT'S LARGEST COST.** Levers, in order: the phase-D bit split, QAT-KD on the leak-free base, then (Andrew: only if it is the only way) more card/note bytes. ⚠ Trained and scored WITH the clock leak, so part of the imm tax may be leak loss; the leak-free QAT arm separates it. Recorded: `research_log.jsonl` (`w10qat`, baseline), `research_5k.md`, verbose "ENDGAME ARM 2", PREREG verdict. **PHASE D (23:09): THE TAX DOES NOT DECOMPOSE BY REMOVAL** -- removing the shift quantizer throws the model OFF DISTRIBUTION (+0.035 / +0.043 on the probe, arm skipped); removing the WKV codebook makes it slightly WORSE (C = -0.000135 / -0.000246, p 1e-4 on imm): the learned quantizers CO-ADAPTED with the weights. The pre-registered falsifier fires (C < 30% of T): **zero-byte catalog routes exhausted, the bit-split A/B is DEAD, the leak-free QAT arms keep b10 / m2b12; remaining routes = QAT-KD (leak-free base), then Andrew's byte grant (rank-2)**. `scratchpad/qat_decomp/PREREG.md` verdict. **w10clk's dry run started 23:12:21** (T=1800, clock-fix ON in both workers).
...
**THE QUEUE WAS RE-PLANNED BY HAND at 08:55:** stopped the four leaked-base waiters -- QAT-KD (28684), budget curve (29148), decay length (36528) and the end-of-queue w10lf (3724) -- cmd first, no markers (a note in each log). **The chain is now: arm 2's eval (running since 08:50; probe OK on the learned catalogs; then phase D) -> `w10clk` at T=1800 (3644, automatic) -> cmixgraft (36904) -> `w10lf` (NEW: `scratchpad/w10lf/wait_cmixgraft_then_lf.cmd`, pid 16676, log `wait_lf2.log`, T fixed at 1800, refuses 150 on cmixgraft's 90; stub-tested on nine paths and run 75 s in the foreground).** cmixgraft stays ahead of w10lf because its question is ahead-side (the leak barely touches ahead), and a capacity win would change which architecture the leak-free base should have. **AFTER w10lf, on its WS, all with the flag -- ORDER (my call): the decay-length pair FIRST, then pick the decay length L*, then the catalog refit + a leak-free QAT arm at L* (the deploy QAT decay AND QAT-KD's control), then QAT-KD at L* (teacher = the leak-free decayed final), then the budget curve LAST** (it cannot change the shipped model). Decay length goes first so the QAT arms need not be re-run at a new length. **BUILT (`e8ebc8a`):** `scratchpad/w10lf/mk_lf_branch.py` -> `lfd3`, `lfd1` (decay length) and `lfb2`, `lfb5` (budget points from `w10lf_ws_22000` / `_55000`, 300-user eval, `verdict_budget.py lf`), each with a dry twin; each differs from w10lf only in decay length or branch step. Their preflight fails today ONLY because w10lf's WS checkpoints do not exist yet -- the waiter must preflight again at fire time. **ARMED 2026-09-11 11:40: `scratchpad/lfd3/wait_lf_then_decaylen.cmd` (pid 37544, log `lfd3/wait_lfdecay.log`, generated by `mk_wait_lf_decaylen.py`)** -- fires on `wait_lf2.log`; refuses 160 on its 150 (arm 2's eval failed), 161 if w10lf never ran, 162 unless `w10lf.log` has its own `DONE_EXIT_0` AND both final checkpoints exist; then preflights the four runners (163), both dry runs (164-169), lfd3, lfd1. Stub-tested on eleven paths, 75 s live. **STILL TO BUILD:** the QAT-lf and QAT-KD-lf runners and the whole post-w10lf waiter chain -- after arm 2's verdict and phase D, which set the QAT recipe. Why re-base rather than keep the ws10 versions: quantizing card/note state can strip the precision the leak needs, so a leaked model's imm QAT tax can be too large; and part of the budget's +0.0015 imm may be the model learning the leak better. ⚠ **Read arm 2's imm tax with that caveat.**
