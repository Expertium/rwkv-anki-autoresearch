@echo off
REM ======================================================================================
REM E2S -- INTERVAL ARM: featA2's recipe on END-TO-START dbs. One variable vs featA2.
REM Generated by mk_features_ab.py. DO NOT HAND-EDIT; regenerate.
REM
REM KD IS OFF, and here the reason is NOT the one in the other arms. These dbs are 92-dim,
REM so the teacher CAN forward-pass them -- which is exactly what makes it dangerous. The KD
REM dump stores the teacher's output logits plus labels_sum as its ONLY identity check, and
REM labels are RATINGS, which the interval definition does not touch. The checksum would
REM therefore PASS while the student distilled toward predictions the teacher computed on
REM end-to-END inputs. Same shape as the augmentation/KD incompatibility: the checksum proves
REM LABEL alignment and gets read as proving BATCH alignment.
REM The control (featA2) is also KD-off, so the comparison stays single-variable.
REM
...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\features_ab\e2s
set LOG=%DIR%\e2s.log
...
echo ===== E2S START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/features_ab/e2s/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\e2s_ws_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/features_ab/e2s e2s_ws e2s_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_e2s 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% DSETUP_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 %DATE% %TIME% >> "%LOG%"
  exit /b 22
)
findstr /C:"e2s_ws_%STEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\e2s_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/features_ab/e2s e2s_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
