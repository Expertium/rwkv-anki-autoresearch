@echo off
REM ======================================================================================
REM FEATB -- TREATMENT: same recipe on the REBUILT 112-dim DBs, 21 real-timestamp features.
...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\features_ab\featB
set LOG=%DIR%\featB.log
...
echo ===== FEATB START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/features_ab/featB/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\featB_ws_%STEPS%.pth" (
...
findstr /C:"featB_ws_%STEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\featB_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/features_ab/featB featB_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
