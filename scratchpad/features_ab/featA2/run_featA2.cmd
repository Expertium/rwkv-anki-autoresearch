@echo off
REM ======================================================================================
REM FEATA2 -- FIXED CONTROL: featA's recipe on the id-fixed published dbs.
...
echo ===== FEATA2 START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/features_ab/featA2/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\featA2_ws_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/features_ab/featA2 featA2_ws featA2_d %DIR%\decay.toml train_db_5k_h1_fix 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% DSETUP_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 %DATE% %TIME% >> "%LOG%"
  exit /b 22
)
findstr /C:"featA2_ws_%STEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\featA2_d_%STEPS%.pth" (
