@echo off
REM ======================================================================================
REM FEATA -- CONTROL: champion recipe with KD OFF on the OLD 92-dim DBs.
...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\features_ab\featA
set LOG=%DIR%\featA.log
...
set RWKV_VAL_DB=F:/rwkv_lmdb/test_db_5k
set RWKV_EVAL_DB=F:/rwkv_lmdb/test_db_5k
...
echo ===== FEATA START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/features_ab/featA/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\featA_ws_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/features_ab/featA featA_ws featA_d %DIR%\decay.toml train_db_5k_h1 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% DSETUP_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 %DATE% %TIME% >> "%LOG%"
  exit /b 22
)
findstr /C:"featA_ws_%STEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\featA_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/features_ab/featA featA_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
findstr /C:"F:/rwkv_lmdb/test_db_5k" "%DIR%\eval.toml" >nul
if not %ERRORLEVEL%==0 (
