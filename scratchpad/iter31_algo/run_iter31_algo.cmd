...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\iter31_algo
set LOG=%DIR%\iter31_algo.log
...
echo ===== iter31_algo START %DATE% %TIME% ===== > "%LOG%"

echo === STEP 0.5: 40-step E2E sanity (PAVA probes + GRU N=3 + Muon wiring, VRAM) %TIME% === >> "%LOG%"
set RWKV_MAX_STEPS=40
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/iter31_algo/iter31_algo_ws.toml > "%DIR%\sanity_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
set RWKV_STEP_TRACE=scratchpad/iter31_algo/iter31_algo_ws_trace.jsonl
set RWKV_GRAD_STATS=scratchpad/iter31_algo/iter31_grad_stats_ws.json
del /Q scratchpad\iter31_algo\iter31_algo_ws_trace.jsonl scratchpad\iter31_algo\iter31_algo_ws_trace.jsonl.val.jsonl 2>nul

echo === WS 1 epoch (1-5000, A18 trunk + PAVA + GRU3 + Muon, vprune ON min6000) %TIME% === >> "%LOG%"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/iter31_algo/iter31_algo_ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if %ERRORLEVEL%==42 (
...
echo WS OK %TIME% >> "%LOG%"
set RWKV_STEP_TRACE=
set RWKV_GRAD_STATS=scratchpad/iter31_algo/iter31_grad_stats_decay.json

echo === DECAY SETUP (0.25 ep, MAX=32768) %TIME% === >> "%LOG%"
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/iter31_algo iter31ws iter31d scratchpad/iter31_algo/iter31_algo_decay.toml train_db_5k_h1 1 5000 0.25 1e-3 32768 > "%DIR%\decay_setup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo DONE_EXIT_DSETUPFAIL %DATE% %TIME% >> "%LOG%"
  exit /b 4
)
echo === DECAY %TIME% === >> "%LOG%"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/iter31_algo/iter31_algo_decay.toml > "%DIR%\decay_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
del /Q result\RWKV-iter31_algo.jsonl result\RWKV-P-iter31_algo.jsonl result\RWKV-iter31_algo-s0.jsonl result\RWKV-P-iter31_algo-s0.jsonl result\RWKV-iter31_algo.nanskip.jsonl result\RWKV-iter31_algo-s0.nanskip.jsonl 2>nul
echo === WRITE EVAL TOML (VAL 5001-7500) %TIME% === >> "%LOG%"
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/iter31_algo iter31d scratchpad/iter31_algo/iter31_algo_eval.toml RWKV-iter31_algo RWKV-P-iter31_algo 5001 7500 > "%DIR%\eval_toml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo DONE_EXIT_TOMLFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  exit /b 6
)
echo === EVAL (single process, d=80, state-clamp ON) %TIME% === >> "%LOG%"
.venv\Scripts\python.exe -u optimization/eval_sharded.py --config scratchpad/iter31_algo/iter31_algo_eval.toml --shards 1 --solo-threshold 0 --fetch-per-shard 4 --threads-per-shard 7 > "%DIR%\eval_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe optimization/paired_pvalue.py --intersect --cand-ahead result/RWKV-iter31_algo.jsonl --cand-imm result/RWKV-P-iter31_algo.jsonl --champ-ahead result/RWKV-track2_a18.jsonl --champ-imm result/RWKV-P-track2_a18.jsonl > "%DIR%\gate_%STAMP%.log" 2>&1
