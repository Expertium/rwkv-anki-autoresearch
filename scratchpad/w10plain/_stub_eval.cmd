...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10plain\_stubdir
set SRC=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10plain\_stubdir
...
cmd /c exit 0 REM_ scratchpad/w10plain w10p_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
REM stub
REM stub: no delete
cmd /c exit 0 REM_ --config %DIR%\eval.toml --shards 1 --solo-threshold 0 --exclude 6701 --fetch-per-shard 2 --threads-per-shard 7 > "%DIR%\eval_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
