...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\realcyc
set LOG=%DIR%\realcyc.log
...
echo ===== REALCYC START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/realcyc/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\rc_ws_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/realcyc rc_ws rc_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\rc_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/realcyc rc_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
