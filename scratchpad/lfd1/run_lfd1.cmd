@echo off
REM lfd1 -- DECAY-LENGTH branch: a 1-epoch decay from w10lf_ws_109350, full VAL-half eval. Generated by scratchpad/w10lf/mk_lf_branch.py from arm 1's runner, with the
...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\lfd1
set LOG=%DIR%\lfd1.log
...
echo ===== lfd1 START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE B: the 2-EPOCH decay from the shared 10-epoch WS checkpoint ----
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/w10lf w10lf_ws lfd1_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10lf\lfd1_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/lfd1 lfd1_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
