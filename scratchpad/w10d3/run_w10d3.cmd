...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10d3
set LOG=%DIR%\w10d3.log
...
echo ===== w10d3 START %DATE% %TIME% ===== > "%LOG%"
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/ws10 w10_ws w10d3_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 3 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/w10d3 w10d3_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
