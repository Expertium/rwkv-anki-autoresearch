@echo off
REM Profile the q72u-era step, two configs, sequential:
REM  (1) QAT: the FROZEN 5k-family env (NO_JIT + sanctioned flags + full q72u learnable-cb QAT)
REM  (2) PLAIN: no QAT env, stock JIT path (the "no QAT phase" step)
REM Profiler = RWKV_PROFILE_STEP=40 RWKV_PROFILE_COUNT=10 -> bucketed GPU self-time, then exit.
cd /d C:\Users\Andrew\rwkv-anki-autoresearch
call "C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvars64.bat" > nul
set LOG=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\prof_q72u\prof.log
...
echo ===== PROF START %DATE% %TIME% ===== > "%LOG%"

echo === QAT (frozen env) %TIME% === >> "%LOG%"
...
echo [QAT_EXIT_%ERRORLEVEL%] %DATE% %TIME% >> "%LOG%"
del /Q scratchpad\prof_q72u\profq_*.pth 2>nul

echo === PLAIN (stock JIT) %TIME% === >> "%LOG%"
...
set RWKV_QAT_ROT_CACHE=
set RWKV_QAT_FAST_EMB=
set RWKV_QAT_EMA_FOREACH=
set RWKV_QAT_NO_MEMFILL=
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/prof_q72u/prof_ws.toml >> "%LOG%" 2>&1
echo [PLAIN_EXIT_%ERRORLEVEL%] %DATE% %TIME% >> "%LOG%"
del /Q scratchpad\prof_q72u\profq_*.pth 2>nul

echo PROF_DONE %DATE% %TIME% >> "%LOG%"
