"""Smoke test for RWKV_GRUP_HEAD=N (track-2 A3, Andrew 2026-07-17): GRU-P-faithful curve
...
Checks (grup ON): param count (dummies + 3 tiny heads replace w_linear + ahead head),
num_curves override, prior-curve init (uniform w, exp(bias) S, d=0.5), zero residual,
monotone-decreasing curve in t, squash bounds, grad flow (randomize the zero-init grup
...
N = int(os.environ.get("RWKV_GRUP_HEAD", "0"))
torch.manual_seed(0)
...
assert model.no_ahead_residual, "grup must force no_ahead_residual"
...
s0 = torch.exp(model.grup_s_bias)
...
with torch.no_grad():
    for pn in ("grup_w_weight", "grup_s_weight", "grup_d_weight"):
...
for t in ts:
    ...
    cv = model.grup_forgetting_curve(w2, s2, d2, tv)
...
# --- grad flow: curve loss reaches grup heads + trunk; dummies stay dead
tv = torch.full((4, 7, 1), 86400.0)
loss = model.grup_forgetting_curve(w2, s2, d2, tv).sum() + p2.sum()
loss.backward()
for pn in ("grup_w_weight", "grup_s_weight", "grup_d_weight",
           "grup_w_bias", "grup_s_bias", "grup_d_bias"):
...
print("grad flow PASS (grup heads + head_w + trunk live; dummies dead)")
...
rnn.eval()
assert rnn.grup_on and rnn.num_curves == N and rnn.no_ahead_residual
...
c_f = model.grup_forgetting_curve(r_w, r_s, r_d, t1)
c_r = rnn.grup_forgetting_curve(r_w, r_s, r_d, t1)
assert torch.equal(c_f, c_r), "fwd vs rnn grup curve mismatch"
...
p32_on = run({**D32, "RWKV_GRUP_HEAD": "2"}, "d32 GRUP N=2, JIT")
p128_off = run(D128, "d128/A1 OFF (param anchor)")
p128_on = run({**D128, "RWKV_GRUP_HEAD": "2"}, "d128/A1 GRUP N=2, JIT")
run({**D128, "RWKV_GRUP_HEAD": "2", "RWKV_NO_JIT": "1"}, "d128/A1 GRUP N=2, NO_JIT")
...
assert p32_on == exp32, f"d32 grup params {p32_on} != expected {exp32}"
assert p128_on == exp128, f"d128 grup params {p128_on} != expected {exp128}"
