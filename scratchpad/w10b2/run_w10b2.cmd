...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10b2
set LOG=%DIR%\w10b2.log
...
echo ===== w10b2 START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE B: the 2-EPOCH decay from the shared 10-epoch WS checkpoint ----
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/ws10 w10_ws w10b2_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 2.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\ws10\w10b2_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/w10b2 w10b2_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 5300 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
