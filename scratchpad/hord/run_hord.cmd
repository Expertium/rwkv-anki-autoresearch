@echo off
REM hord -- generated by scratchpad/hord/mk_hord.py from the realcyc runner. DO NOT HAND-EDIT.
REM ======================================================================================
...
set RWKV_PAVA_HORIZON_LAMBDA=0.05

if not exist "%DIR%" mkdir "%DIR%"
echo ===== hord START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/hord/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\hd_ws_%STEPS%.pth" (
...
findstr /C:"[pava-horizon] button-order hinge at t x [0.125, 8.0] ON: lambda=0.05" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/hord hd_ws hd_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% DSETUP_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 %DATE% %TIME% >> "%LOG%"
  exit /b 22
)
findstr /C:"hd_ws_%STEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\hd_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/hord hd_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
