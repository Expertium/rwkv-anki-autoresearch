@echo off
REM ===========================================================================================
REM HYBRID ARM C: d=32, context 1/1/1/1, features_fc_mult=12 -- feature MLP at 48% of budget
REM Priced at 99596 params vs the champion's 558,212. Design: optimization/HYBRID_100K.md.
REM
...
REM With 99596 params the budget is dparams == 458,616, so the largest tolerable
REM regression is about 0.000459 in each mode. For scale the A0-to-A18 ladder scored
...
set RWKV_GRAD_STATS=%DIR%\grad_stats_hyc.json
set RWKV_ID_FEATURES=
set RWKV_ABLATE_FEATURES=
set RWKV_MAX_STEPS=

echo ===== HYBRID ARM C (99596 params) START %DATE% %TIME% ===== >> "%LOG%"
...
if not %ERRORLEVEL%==0 (
  echo hybC SCRIPTED_EVAL_SMOKE_FAILED %DATE% %TIME% >> "%LOG%"
...
set RWKV_ARCH_MODULE=scratchpad/hybrid100k/arch_C.py
set RWKV_STRIP_CMIX=card_id:1,preset_id:0,user_id:0
echo hybC SMOKE_OK %TIME% >> "%LOG%"

REM ================= PHASE 0b: is this the model we priced? =================
.venv\Scripts\python.exe scratchpad/hybrid100k/assert_arch.py 99596 > "%DIR%\arch_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybC ARCH_ASSERT_FAILED %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_46 >> "%LOG%"
  exit /b 46
)
echo hybC ARCH_OK 99596 params %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/hybrid100k/hyc_ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybC WSFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_21 >> "%LOG%"
  exit /b 21
)
findstr /C:"[kd-mix] KD ON" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybC NOKD_WS %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_35 >> "%LOG%"
  exit /b 35
)
findstr /C:"alpha FIXED at 0.9" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybC WRONGALPHA_WS %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_36 >> "%LOG%"
  exit /b 36
)
REM Gate on the ARTIFACT, not the exit code: train_rwkv can swallow a fatal error and exit 0.
if not exist "%DIR%\hyc_ws_%WSSTEPS%.pth" (
  echo hybC NO_WS_CKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_30 >> "%LOG%"
  exit /b 30
)
echo hybC WS_OK %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/hybrid100k hyc_ws hyc_d %DIR%\hyc_decay.toml train_db_5k_h1 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybC DSETUPFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 >> "%LOG%"
  exit /b 22
)
findstr /C:"hyc_ws_%WSSTEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybC WRONGCKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_32 >> "%LOG%"
  exit /b 32
)
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config %DIR%\hyc_decay.toml > "%DIR%\decay_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybC DECAYFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_23 >> "%LOG%"
  exit /b 23
)
findstr /C:"alpha FIXED at 0.5" "%DIR%\decay_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybC WRONGALPHA_DECAY %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_36 >> "%LOG%"
  exit /b 36
)
if not exist "%DIR%\hyc_d_%WSSTEPS%.pth" (
  echo hybC NO_DECAY_CKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_31 >> "%LOG%"
  exit /b 31
)
echo hybC DECAY_OK %TIME% >> "%LOG%"

REM ================= PHASE C: rectified VAL-half eval =================
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/hybrid100k hyc_d %DIR%\hyc_eval.toml RWKV-hybC RWKV-P-hybC 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybC TOMLFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe -u optimization/eval_sharded.py --config %DIR%\hyc_eval.toml --shards 1 --solo-threshold 0 --fetch-per-shard 2 --threads-per-shard 7 > "%DIR%\eval1_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybC EVAL attempt 1 failed - retrying from banked users %TIME% >> "%LOG%"
  .venv\Scripts\python.exe -u optimization/eval_sharded.py --config %DIR%\hyc_eval.toml --shards 1 --solo-threshold 0 --fetch-per-shard 2 --threads-per-shard 7 > "%DIR%\eval2_%STAMP%.log" 2>&1
)
if not %ERRORLEVEL%==0 (
  echo hybC EVALFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_25 >> "%LOG%"
  exit /b 25
)
echo hybC EVAL_OK %TIME% >> "%LOG%"
