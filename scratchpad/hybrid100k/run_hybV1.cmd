@echo off
REM ===========================================================================================
REM HYBRID ARM V1: FSRS-7 (S_long, S_short, D) replaces the card stream's WKV; the trunk EMITS the 34 parameters per review
REM Priced at 488858 params vs the champion's 558,212. Design: optimization/HYBRID_100K.md.
REM
REM GATE: CLAUDE.md's SIZE/SPEED EXCEPTION, pre-registered 2026-08-30 BEFORE
...
REM With 488858 params the budget is dparams == 69,354, so the largest tolerable
REM regression is about 0.000069 in each mode. For scale the A0-to-A18 ladder scored
...
set RWKV_GRAD_STATS=%DIR%\grad_stats_hyv1.json
set RWKV_ID_FEATURES=
set RWKV_ABLATE_FEATURES=
set RWKV_MAX_STEPS=

echo ===== HYBRID ARM V1 (488858 params) START %DATE% %TIME% ===== >> "%LOG%"
...
if not %ERRORLEVEL%==0 (
  echo hybV1 SCRIPTED_EVAL_SMOKE_FAILED %DATE% %TIME% >> "%LOG%"
...
set RWKV_STRIP_CMIX=user_id:0,user_id:1,user_id:2,preset_id:0,preset_id:1,preset_id:2,deck_id:1,deck_id:2
set RWKV_FSRS_CARD=0
echo hybV1 SMOKE_OK %TIME% >> "%LOG%"

REM ================= PHASE 0b: is this the model we priced? =================
.venv\Scripts\python.exe scratchpad/hybrid100k/assert_arch.py 488858 > "%DIR%\arch_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybV1 ARCH_ASSERT_FAILED %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_46 >> "%LOG%"
  exit /b 46
)
echo hybV1 ARCH_OK 488858 params %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/hybrid100k/hyv1_ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybV1 WSFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_21 >> "%LOG%"
  exit /b 21
)
findstr /C:"[kd-mix] KD ON" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybV1 NOKD_WS %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_35 >> "%LOG%"
  exit /b 35
)
findstr /C:"alpha FIXED at 0.9" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybV1 WRONGALPHA_WS %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_36 >> "%LOG%"
  exit /b 36
)
REM Gate on the ARTIFACT, not the exit code: train_rwkv can swallow a fatal error and exit 0.
if not exist "%DIR%\hyv1_ws_%WSSTEPS%.pth" (
  echo hybV1 NO_WS_CKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_30 >> "%LOG%"
  exit /b 30
)
echo hybV1 WS_OK %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/hybrid100k hyv1_ws hyv1_d %DIR%\hyv1_decay.toml train_db_5k_h1 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybV1 DSETUPFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 >> "%LOG%"
  exit /b 22
)
findstr /C:"hyv1_ws_%WSSTEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybV1 WRONGCKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_32 >> "%LOG%"
  exit /b 32
)
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config %DIR%\hyv1_decay.toml > "%DIR%\decay_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybV1 DECAYFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_23 >> "%LOG%"
  exit /b 23
)
findstr /C:"alpha FIXED at 0.5" "%DIR%\decay_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybV1 WRONGALPHA_DECAY %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_36 >> "%LOG%"
  exit /b 36
)
if not exist "%DIR%\hyv1_d_%WSSTEPS%.pth" (
  echo hybV1 NO_DECAY_CKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_31 >> "%LOG%"
  exit /b 31
)
echo hybV1 DECAY_OK %TIME% >> "%LOG%"

REM ================= PHASE C: rectified VAL-half eval =================
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/hybrid100k hyv1_d %DIR%\hyv1_eval.toml RWKV-hybV1 RWKV-P-hybV1 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybV1 TOMLFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe -u optimization/eval_sharded.py --config %DIR%\hyv1_eval.toml --shards 1 --solo-threshold 0 --fetch-per-shard 2 --threads-per-shard 7 > "%DIR%\eval1_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybV1 EVAL attempt 1 failed - retrying from banked users %TIME% >> "%LOG%"
  .venv\Scripts\python.exe -u optimization/eval_sharded.py --config %DIR%\hyv1_eval.toml --shards 1 --solo-threshold 0 --fetch-per-shard 2 --threads-per-shard 7 > "%DIR%\eval2_%STAMP%.log" 2>&1
)
if not %ERRORLEVEL%==0 (
  echo hybV1 EVALFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_25 >> "%LOG%"
  exit /b 25
)
echo hybV1 EVAL_OK %TIME% >> "%LOG%"
