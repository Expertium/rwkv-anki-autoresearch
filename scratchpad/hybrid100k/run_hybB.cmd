@echo off
REM ===========================================================================================
REM HYBRID ARM B: d=32, context 1/2/2/2, features_fc_mult=8 -- feature MLP at 32% of budget
REM Priced at 100263 params vs the champion's 558,212. Design: optimization/HYBRID_100K.md.
REM
...
REM With 100263 params the budget is dparams == 457,949, so the largest tolerable
REM regression is about 0.000458 in each mode. For scale the A0-to-A18 ladder scored
...
set RWKV_ARCH_MODULE=scratchpad/hybrid100k/arch_B.py
...
set RWKV_GRAD_STATS=%DIR%\grad_stats_hyb.json
set RWKV_ID_FEATURES=
set RWKV_ABLATE_FEATURES=
set RWKV_MAX_STEPS=

echo ===== HYBRID ARM B (100263 params) START %DATE% %TIME% ===== >> "%LOG%"
...
if not %ERRORLEVEL%==0 (
  echo hybB SCRIPTED_EVAL_SMOKE_FAILED %DATE% %TIME% >> "%LOG%"
...
set RWKV_ARCH_MODULE=scratchpad/hybrid100k/arch_B.py
set RWKV_STRIP_CMIX=card_id:1,deck_id:1,preset_id:0,preset_id:1,user_id:0,user_id:1
echo hybB SMOKE_OK %TIME% >> "%LOG%"

REM ================= PHASE 0b: is this the model we priced? =================
.venv\Scripts\python.exe scratchpad/hybrid100k/assert_arch.py 100263 > "%DIR%\arch_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybB ARCH_ASSERT_FAILED %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_46 >> "%LOG%"
  exit /b 46
)
echo hybB ARCH_OK 100263 params %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/hybrid100k/hyb_ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybB WSFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_21 >> "%LOG%"
  exit /b 21
)
findstr /C:"[kd-mix] KD ON" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybB NOKD_WS %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_35 >> "%LOG%"
  exit /b 35
)
findstr /C:"alpha FIXED at 0.9" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybB WRONGALPHA_WS %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_36 >> "%LOG%"
  exit /b 36
)
REM Gate on the ARTIFACT, not the exit code: train_rwkv can swallow a fatal error and exit 0.
if not exist "%DIR%\hyb_ws_%WSSTEPS%.pth" (
  echo hybB NO_WS_CKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_30 >> "%LOG%"
  exit /b 30
)
echo hybB WS_OK %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/hybrid100k hyb_ws hyb_d %DIR%\hyb_decay.toml train_db_5k_h1 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybB DSETUPFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 >> "%LOG%"
  exit /b 22
)
findstr /C:"hyb_ws_%WSSTEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybB WRONGCKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_32 >> "%LOG%"
  exit /b 32
)
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config %DIR%\hyb_decay.toml > "%DIR%\decay_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybB DECAYFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_23 >> "%LOG%"
  exit /b 23
)
findstr /C:"alpha FIXED at 0.5" "%DIR%\decay_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
  echo hybB WRONGALPHA_DECAY %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_36 >> "%LOG%"
  exit /b 36
)
if not exist "%DIR%\hyb_d_%WSSTEPS%.pth" (
  echo hybB NO_DECAY_CKPT %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_31 >> "%LOG%"
  exit /b 31
)
echo hybB DECAY_OK %TIME% >> "%LOG%"

REM ================= PHASE C: rectified VAL-half eval =================
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/hybrid100k hyb_d %DIR%\hyb_eval.toml RWKV-hybB RWKV-P-hybB 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybB TOMLFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
...
.venv\Scripts\python.exe -u optimization/eval_sharded.py --config %DIR%\hyb_eval.toml --shards 1 --solo-threshold 0 --fetch-per-shard 2 --threads-per-shard 7 > "%DIR%\eval1_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo hybB EVAL attempt 1 failed - retrying from banked users %TIME% >> "%LOG%"
  .venv\Scripts\python.exe -u optimization/eval_sharded.py --config %DIR%\hyb_eval.toml --shards 1 --solo-threshold 0 --fetch-per-shard 2 --threads-per-shard 7 > "%DIR%\eval2_%STAMP%.log" 2>&1
)
if not %ERRORLEVEL%==0 (
  echo hybB EVALFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_25 >> "%LOG%"
  exit /b 25
)
echo hybB EVAL_OK %TIME% >> "%LOG%"
