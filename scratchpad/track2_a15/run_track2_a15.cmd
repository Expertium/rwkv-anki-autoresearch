...
REM 2026-07-25: 808,762 params = -41.4% vs A14 (1,380,660) = 3.41x below the
REM original 2.76M; per-card state 6,528 vs 8,704 floats (-25%).
REM GATE: ratio vs A14 on VAL 5001-7500 -- dparams 571,898 => allowed +0.000572
...
REM VPRUNE_MIN_STEP=6000, ref = champion_5k_track2.json (= A14). Launch DETACHED
REM via detach.ps1 with ABSOLUTE path (CRLF file!).
REM ============================================================================
cd /d C:\Users\Andrew\rwkv-anki-autoresearch
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\track2_a15
set LOG=%DIR%\track2_a15.log
...
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/track2_a15/track2_a15_ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if %ERRORLEVEL%==42 (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/track2_a15 t2a15ws t2a15d scratchpad/track2_a15/track2_a15_decay.toml train_db_5k_h1 1 5000 0.25 1e-3 32768 > "%DIR%\decay_setup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo DONE_EXIT_DSETUPFAIL %DATE% %TIME% >> "%LOG%"
  exit /b 4
)
echo === DECAY %TIME% === >> "%LOG%"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/track2_a15/track2_a15_decay.toml > "%DIR%\decay_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/track2_a15 t2a15d scratchpad/track2_a15/track2_a15_eval.toml RWKV-track2_a15 RWKV-P-track2_a15 5001 7500 > "%DIR%\eval_toml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo DONE_EXIT_TOMLFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  exit /b 6
)
echo === EVAL (single process, d=96, state-clamp ON) %TIME% === >> "%LOG%"
.venv\Scripts\python.exe -u optimization/eval_sharded.py --config scratchpad/track2_a15/track2_a15_eval.toml --shards 1 --solo-threshold 0 --fetch-per-shard 4 --threads-per-shard 7 > "%DIR%\eval_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
echo === GATE: paired vs CHAMPION A14 (val half; ratio gate judged at record time) %TIME% === >> "%LOG%"
.venv\Scripts\python.exe optimization/paired_pvalue.py --intersect --cand-ahead result/RWKV-track2_a15.jsonl --cand-imm result/RWKV-P-track2_a15.jsonl --champ-ahead result/RWKV-track2_a14.jsonl --champ-imm result/RWKV-P-track2_a14.jsonl > "%DIR%\gate_%STAMP%.log" 2>&1
