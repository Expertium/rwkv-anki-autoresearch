@echo off
REM durdrop -- generated by scratchpad/durdrop/mk_durdrop.py from the realcyc runner. DO NOT HAND-EDIT.
REM ======================================================================================
...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\durdrop
set LOG=%DIR%\durdrop.log
...
set RWKV_DUR_DROP=0.25

if not exist "%DIR%" mkdir "%DIR%"
echo ===== durdrop START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/durdrop/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\dd_ws_%STEPS%.pth" (
...
findstr /C:"[dur-drop] TRAIN-ONLY Bernoulli zeroing of input dim 8 (scaled_duration) with p=0.25" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/durdrop dd_ws dd_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% DSETUP_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 %DATE% %TIME% >> "%LOG%"
  exit /b 22
)
findstr /C:"dd_ws_%STEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\dd_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/durdrop dd_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
