@echo off
REM lfb5dry -- BUDGET-CURVE point: a 2-epoch decay from w10lf_ws_55000 (5.03 epochs of WS), 300-user eval. Generated by scratchpad/w10lf/mk_lf_branch.py from arm 1's runner, with the
...
REM DRY RUN: 0.05 epochs of decay and 20 eval users -- plumbing only, never a result.
setlocal
cd /d C:\Users\Andrew\rwkv-anki-autoresearch

set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\lfb5dry
set LOG=%DIR%\lfb5dry.log
...
REM ---- Branch from an INTERMEDIATE w10lf WS checkpoint. Under WSD the stable phase is at constant
REM LR, so this IS a shorter-budget run. Default-unset elsewhere.
set RWKV_DECAY_FROM_STEP=%WSSTEPS%

if not exist "%DIR%" mkdir "%DIR%"
echo ===== lfb5dry START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE B: the 2-EPOCH decay from the shared 10-epoch WS checkpoint ----
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/w10lf w10lf_ws lfb5dry_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 0.05 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10lf\lfb5dry_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/lfb5dry lfb5dry_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 5020 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
