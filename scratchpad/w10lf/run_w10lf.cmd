...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10lf
set LOG=%DIR%\w10lf.log
...
echo ===== w10lf START %DATE% %TIME% ===== > "%LOG%"
...
if exist "%DIR%\w10lf_ws_%WSSTEPS%.pth" goto wsdone
...
if not exist "%DIR%\w10lf_ws_1000.pth" (
  echo %TAG% WS_NO_CHECKPOINT -- launch failure, not a crash %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_21 %DATE% %TIME% >> "%LOG%"
  exit /b 21
)
.venv\Scripts\python.exe scratchpad/make_resume.py %DIR% w10lf_ws scratchpad/w10lf/ws.toml scratchpad/w10lf/ws_resume.toml >> "%LOG%" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% MAKE_RESUME_FAILED %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_26 %DATE% %TIME% >> "%LOG%"
  exit /b 26
)
set CFG=scratchpad/w10lf/ws_resume.toml
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/w10lf w10lf_ws w10lf_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 2.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10lf\w10lf_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/w10lf w10lf_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
