@echo off
REM lfd1dry -- DECAY-LENGTH branch: a 1-epoch decay from w10lf_ws_109350, full VAL-half eval. Generated by scratchpad/w10lf/mk_lf_branch.py from arm 1's runner, with the
...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\lfd1dry
set LOG=%DIR%\lfd1dry.log
...
echo ===== lfd1dry START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE B: the 2-EPOCH decay from the shared 10-epoch WS checkpoint ----
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/w10lf w10lf_ws lfd1dry_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 0.05 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\w10lf\lfd1dry_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/lfd1dry lfd1dry_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 5020 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
