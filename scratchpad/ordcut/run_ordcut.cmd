@echo off
REM ordcut -- generated by scratchpad/ordcut/mk_ordcut.py from the realcyc runner. DO NOT HAND-EDIT.
REM ======================================================================================
...
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\ordcut
set LOG=%DIR%\ordcut.log
...
set RWKV_ORD_LAMBDA=0.25

if not exist "%DIR%" mkdir "%DIR%"
echo ===== ordcut START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/ordcut/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\oc_ws_%STEPS%.pth" (
...
findstr /C:"Trainable parameters: 563654" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
findstr /C:"[ord] ordinal one-cut supervision on the curve logit ON: lambda=0.25" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/ordcut oc_ws oc_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% DSETUP_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 %DATE% %TIME% >> "%LOG%"
  exit /b 22
)
findstr /C:"oc_ws_%STEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\oc_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/ordcut oc_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
