@echo off
REM eqw -- generated by scratchpad/eqw/mk_eqw.py from the realcyc runner. DO NOT HAND-EDIT.
REM ======================================================================================
...
set RWKV_EQUALIZE_LOSS_W=0.25

if not exist "%DIR%" mkdir "%DIR%"
echo ===== eqw START %DATE% %TIME% ===== > "%LOG%"

REM ---- PHASE A: WS, 1 epoch ----
if exist "%DIR%\ws_%STAMP%.log" del /q "%DIR%\ws_%STAMP%.log"
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/eqw/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\eq_ws_%STEPS%.pth" (
...
findstr /C:"[eqw] scored-set loss weighting ON: unscored (label_is_equalize=0) rows weighted 0.25" "%DIR%\ws_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/eqw eq_ws eq_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% DSETUP_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 %DATE% %TIME% >> "%LOG%"
  exit /b 22
)
findstr /C:"eq_ws_%STEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\eq_d_%STEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/eqw eq_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
