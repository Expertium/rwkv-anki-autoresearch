...
REM ============================================================================
REM ### RELAUNCH v2 (2026-07-27). The first launch died at the smoke gate:
REM ###   DUMP_CHECK_FAIL 5/5, 'p_curve outside (0,1): [.., 1.000000]'
REM ### That was a FALSE FAILURE in check_dump.py, not a bad dump. p_curve is stored fp16
REM ### (train_rwkv.py:1090) and fp16 spacing below 1.0 is 4.88e-4, so every teacher output
REM ### above ~0.99951 becomes exactly 1.0 -- 9.97% of values. Harmless: p_curve is consumed
REM ### as a soft BCE TARGET (srs_model.py:772) mixed with hard labels that are themselves
REM ### exactly 0/1, so a 1.0 target gives -log(p), which is finite. check_dump.py now tests
REM ### [0,1] plus an ANTI-COLLAPSE condition (>=10% strictly interior), which is the failure
REM ### that would actually waste the ten hours. Re-verified: DUMP_CHECK_OK, 7.6 GB projected.
REM ### Only the waitlog changed vs v1: parks on mode3 (running) instead of mode2 (done).
cd /d C:\Users\Andrew\rwkv-anki-autoresearch
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\iter32_kd
REM ### v2 writes its OWN log. DEADLOCK GUARD: mode3_noise.cmd is still polling
REM ### iter32_kd.log for DONE_EXIT_, and this script opens its log with ">" which
REM ### TRUNCATES it. Sharing the file would erase the very token mode3 waits for,
REM ### while this script waits on mode3 -- a mutual wait that never resolves.
set LOG=%DIR%\iter32_kd_v2.log
set STAMP=%RANDOM%%RANDOM%
set DUMP=C:\rwkv_kd_dump\t128_iter32
set SMOKE=C:\rwkv_kd_dump\t128_smoke
set WSSTEPS=22346
set WAITLOG=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\eval_pava\mode3_noise.log

echo ===== iter32_kd RELAUNCH (v2) START %DATE% %TIME% ===== > "%LOG%"
...
echo === WAIT for the mode-3 noise control to finish %TIME% === >> "%LOG%"
:waitloop
