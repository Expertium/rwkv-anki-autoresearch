...
REM The dump lands on C:. It was on F: while C: had only 54 GB free; after Andrew authorised
REM deleting the two superseded dbs (2026-08-30) C: has ~246 GB. Measured: F: writes 650 KB files
REM at 47.7/s versus C: at 2,666/s (56x). That was NOT the dump's bottleneck -- the READS were,
REM and the train db has since moved to C: behind a junction -- but the write side is free to fix.
REM
...
set TAG=e2sc
set DIR=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\e2s_rebase
set LOG=%DIR%\e2s_rebase.log
set STAMP=%RANDOM%%RANDOM%
set WSSTEPS=10935
set DUMP=C:\rwkv_kd_dump\t128_e2s_65k
set SMOKE=C:\rwkv_kd_dump\t128_e2s_smoke

if not exist "%DIR%" mkdir "%DIR%"
echo ===== E2S RE-BASE START %DATE% %TIME% ===== > "%LOG%"
...
set RWKV_KD_STEPS=5
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/e2s_rebase/dump.toml > "%DIR%\smoke_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/e2s_rebase/dump.toml > "%DIR%\dump_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
.venv\Scripts\python.exe -u -m rwkv.train_rwkv --config scratchpad/e2s_rebase/ws.toml > "%DIR%\ws_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo WSFAIL_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_22 %DATE% %TIME% >> "%LOG%"
  exit /b 22
)
if not exist "%DIR%\e2sc_ws_%WSSTEPS%.pth" (
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/e2s_rebase e2sc_ws e2sc_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_e2s 1 5000 1.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo DSETUP_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_23 %DATE% %TIME% >> "%LOG%"
  exit /b 23
)
findstr /C:"e2sc_ws_%WSSTEPS%" "%DIR%\dsetup_%STAMP%.log" >nul
if not %ERRORLEVEL%==0 (
...
if not exist "%DIR%\e2sc_d_%WSSTEPS%.pth" (
...
set RWKV_KD_MIX=
set RWKV_KD_ALPHA=
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/e2s_rebase e2sc_d %DIR%\eval.toml RWKV-e2sc RWKV-P-e2sc 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if exist "result\RWKV-e2sc.jsonl" del /q "result\RWKV-e2sc.jsonl"
if exist "result\RWKV-P-e2sc.jsonl" del /q "result\RWKV-P-e2sc.jsonl"
