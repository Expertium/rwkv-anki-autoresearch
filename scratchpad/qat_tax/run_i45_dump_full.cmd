...
REM
REM QAT#2 (PROPOSALS.md QAT sub-queue item 2): the FULL 10,935-step teacher dump from the PLAIN
REM iter-45 champion, so a quant-aware run can distil from 'what the unquantised model predicts'
REM -- which IS the deploy objective (minimise the QAT tax), rather than merely low logloss.
REM Replaces the d=128 teacher for cleaner attribution, per the proposal.
REM The 200-step ours_i45_immcorr dump proved the batch stream matches (its step-1 labels_sum is
REM identical to t128_seedpair_65k's), so this is the same thing at full length, LABELS omitted
REM to keep it near 7 GB.
...
set LOG=%DIR%\i45_full_dump.log
set OUTDIR=C:\rwkv_kd_dump\ours_i45_full
...
echo I45FULLDUMP_DONE_EXIT_0 %DATE% %TIME% >> "%LOG%"
endlocal & exit /b 0
