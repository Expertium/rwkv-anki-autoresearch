...
echo ===== w10qatkd START %DATE% %TIME% ===== > "%LOG%"
...
.venv\Scripts\python.exe scratchpad/write_decay_setup.py scratchpad/ws10 w10_ws w10qk_d %DIR%\decay.toml F:/rwkv_lmdb/train_db_5k_h1_id5 1 5000 2.0 1e-3 65536 > "%DIR%\dsetup_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
if not exist "C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\ws10\w10qk_d_%STEPS%.pth" (
...
set RWKV_QAT_PQ=scratchpad/ws10/w10qk_d_wkvcb_%STEPS%.txt
set RWKV_QAT_SHIFT_PQ=scratchpad/ws10/w10qk_d_shiftcb_%STEPS%.txt
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/w10qatkd w10qk_d %DIR%\probe.toml RWKV-%TAG%p RWKV-P-%TAG%p 5001 5010 > "%DIR%\ptoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% PROBE_TOML_FAILED %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_68 %DATE% %TIME% >> "%LOG%"
  exit /b 68
)
findstr /C:"w10qk_d_%STEPS%.pth" "%DIR%\probe.toml" >nul
if errorlevel 1 (
...
findstr /C:"[QAT-PQ] uploaded rank-1 codebook scratchpad/ws10/w10qk_d_wkvcb_%STEPS%.txt" "%DIR%\probe_%STAMP%.log" >nul
if errorlevel 1 (
  echo %TAG% PROBE_DID_NOT_LOAD_THE_LEARNED_WKV_CATALOG %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_72 %DATE% %TIME% >> "%LOG%"
  exit /b 72
)
findstr /C:"[QAT-SHIFT-PQ] loaded scratchpad/ws10/w10qk_d_shiftcb_%STEPS%.txt" "%DIR%\probe_%STAMP%.log" >nul
if errorlevel 1 (
...
.venv\Scripts\python.exe scratchpad/write_eval_toml.py scratchpad/w10qatkd w10qk_d %DIR%\eval.toml RWKV-%TAG% RWKV-P-%TAG% 5001 7500 > "%DIR%\etoml_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
...
findstr /C:"w10qk_d_wkvcb_%STEPS%.txt" scratchpad\eval_shards\shard_*.log >nul 2>&1
if errorlevel 1 (
