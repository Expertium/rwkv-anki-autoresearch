@echo off
REM ======================================================================================
REM gen4base EVAL, FALLBACK: the remaining VAL users WITHOUT 6701 (phase C only), sliced from run_gen4base_p2.cmd after the eval OOM'd at 04:50 on
...
REM ---- PHASE C (FALLBACK): users 5001-7500 minus 6701 via USERS_FILE; get_result skips the 1700 banked ----
REM User 6701's million-row chunks need more than the WDDM ceiling (12 GB VRAM + 32 GB shared) once the gen-4
REM equalize set's probe rows are added; three attempts OOM'd at the identical 36.09 GiB. Excluded from the
REM gen-4/gen-5 lineage's VAL set; every candidate pairs on the intersection, so the gate is unaffected.
set RWKV_EVAL_PAVA=1
findstr /C:"F:/rwkv_lmdb/test_db_5k_id4" "%DIR%\skip6701\eval_rest.toml" >nul
if not %ERRORLEVEL%==0 (
  echo %TAG% EVAL_DB_MISMATCH %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_34 %DATE% %TIME% >> "%LOG%"
  exit /b 34
)
.venv\Scripts\python.exe -u -m rwkv.get_result --config scratchpad/gen4_base/skip6701/eval_rest.toml > "%DIR%\evalrest_%STAMP%.log" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% EVAL_FAILED_%ERRORLEVEL% %DATE% %TIME% >> "%LOG%"
  echo DONE_EXIT_25 %DATE% %TIME% >> "%LOG%"
  exit /b 25
)
.venv\Scripts\python.exe scratchpad/gen4_base/skip6701/merge_skip6701.py >> "%LOG%" 2>&1
if not %ERRORLEVEL%==0 (
  echo %TAG% MERGE_FAILED %DATE% %TIME% >> "%LOG%"
