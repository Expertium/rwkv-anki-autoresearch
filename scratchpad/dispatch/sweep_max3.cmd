...
set EVALLOG=C:\Users\Andrew\rwkv-anki-autoresearch\scratchpad\features_ab\featA2\featA2_evalonly.log
...
echo ===== MAX SWEEP v3 START %DATE% %TIME% ===== >> "%LOG%"
...
REM ---- phase 1c: the featA2 eval owns the GPU for about 2.3 h. A bench run beside it
REM      measures contention, not MAX. That eval STARTS BEFORE the rebuild ends, so
REM      waiting on the rebuild alone is not enough -- which is why v2 was replaced.
echo --- waiting for the featA2 eval to finish %TIME% >> "%LOG%"
:waiteval
findstr /B /C:"DONE_EXIT_" "%EVALLOG%" >nul 2>&1
if errorlevel 1 (
  ping -n 121 127.0.0.1 >nul
  goto waiteval
)
echo --- featA2 eval finished %TIME% >> "%LOG%"
...
for %%M in (65536 49152 40960 32768 24576 65536) do (
  ...
  .venv\Scripts\python.exe -u -m rwkv.train_rwkv --config "%DIR%\ws_max%%M.toml" > "%DIR%\v3_%%M_%STAMP%.log" 2>&1
  set RC=!ERRORLEVEL!
  findstr /C:"BENCH_RESULT" "%DIR%\v3_%%M_%STAMP%.log" >> "%LOG%"
